...
class DeepLabv3_plus(nn.Module):
    def __init__(self, nInputChannels=3, n_classes=21, os=16, pretrained=False, _print=True):
        ...
        self.conv2 = nn.Conv2d(128+2, 48, 1, bias=False)
        ...
        self.drop = nn.Dropout2d(p=0.2)   #SC
    ...
    def forward(self, inputs):
        ...
        dim0 = low_level_features.size(0)
        dim1 = low_level_features.size(1)
        dim2 = low_level_features.size(2)
        dim3 = low_level_features.size(3)

        f1 = torch.ones([dim0, 1, dim2, dim3],dtype=torch.float32)
        f2 = torch.ones([dim0, 1, dim2, dim3],dtype=torch.float32)
        for i in range(salt_props.size(0)):
            f1[i,:,:,:] = salt_props[i]
            f2[i,:,:,:] = depths[i]
        ...
        low_level_features = torch.cat((low_level_features, f1, f2), dim=1)
