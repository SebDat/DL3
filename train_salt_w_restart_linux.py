...
nEpochs = 99  # Number of epochs for training
resume_epoch = 99   # Default is 0, change if want to resume

p = OrderedDict()  # Parameters to include in report
p['trainBatch'] = 32  # Training batch size
testBatch = 32  # Testing batch size
...
LabelsNumpy = np.zeros((num_img_ts,1,sz,sz))
PredNumpy = np.zeros((num_img_ts,1,sz,sz))
...
for ii, sample_batched in enumerate(testloader):
    ...
    LabelsNumpy[c:c+testBatch,1,:,:] = labels.numpy()
    PredNumpy[c:c+testBatch,1,:,:] = outputs.numpy()
...
IOU = iou_metric_batch(np.round(y_valid).astype(int).squeeze(axis = 3),np.round(pred).astype(int).squeeze(axis = 3))
